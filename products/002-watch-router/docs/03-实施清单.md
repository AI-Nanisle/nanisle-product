...
| M4 = C5–C7 | 线上 Lambda 实抓 3 条 YouTube 长视频字幕全通;拔掉代理验证兜底路由真的接手 | ◐ 验证进行中:代理注入后云端任务已 `path=subtitle`(注入前 AWS IP 全拦),3 条 YouTube + 1 条 B站 whisper 样本在跑;「拔代理验兜底」与 C6 失败路由未做 |
...
1. **I 线交互包**(I1–I3,约 2h):001 深读链接 + interop 端点 + 002 用户级高亮——M6 的全部内容,也是 002 从「独立小岛」变成与 001 成环的一步。**下一个动工项**
2. **C6 · 失败路由**(约 1h):代理重试 2 次 → TranscriptAPI/Supadata → whisper;按平台记成功/失败与路径进 CloudWatch(02 风险 2 说的运营指标——YouTube 可用性要当指标盯,这是那个仪表的落点);顺带补 M4 的「拔代理验兜底」验证
3. **播客 enclosure 样本**(M3 最后一角):等手头有真实播客单集/音频直链时投一条;B站 whisper 样本已在本轮验证套餐里
4. 单测基建(vitest)+ W6 锚定/W8 归一的边界样例(清单备注挂着的欠账)
5. 运营观察项(不排期):CF 出口 IP 快车道成功率、Webshare 流量消耗(1GB 池)、DeepSeek 对转写的编辑质量——三者进 002 主站文案「学到了什么」的素材池
