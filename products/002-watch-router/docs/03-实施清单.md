...
- [ ] **F1 · 收单页**:首页即输入框(URL 或粘贴正文),读 `?url=` query 预填(I1 深读入口的承接端);提交后按车道跳流式结果页或进度页
- [ ] **F2 · 快车道流式渲染**:SSE 透传边生成边渲染;抽取失败态给「把正文粘贴进来」的输入退路(T2 三级兜底的最后一级)
- [ ] **F3 · 结果页**(T4):判决一句话;总体要点(每条带 quote,命中的可跳转);分段地图(覆盖全程,`value: low` 灰显 + 顶部「低价值段共 N 分钟」统计,`tracked` 高亮待 I3);路径徽章(官方字幕/whisper/文章/粘贴);`truncated` 时标注「内容过长,已按前 N 处理」
- [ ] **F4 · 跳转落地**(T4):B站/YouTube 带秒数 URL 直达;播客展示 `mm:ss`;文章 `start` 为段落锚点,点击滚动到 002 存的抽取正文对应位置并高亮
- [ ] **F5 · 低置信渲染**(T4):锚定校验没配上的要点灰显 + 「未能在原文中定位」标注,不可跳转
- [ ] **F6 · 进度页**:轮询 `GET /api/task/:id`,五步进度 + 路径徽章;失败态文案区分「超时」「转写失败」「配额用完」,超时/失败引导粘贴 transcript 的终极保底
- [ ] **F7 · 配额读数**:页眉当日提交额度读数(001 Q3 同款形态,上限由服务端下发)
