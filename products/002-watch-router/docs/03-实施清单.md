...
> 状态:2026-08-25 从 [02-技术方案.md](02-技术方案.md)(多用户版,含文首修订 ④)拆出;**2026-08-26 六线中 D/W/F/P 全部完成、C 线代码就位并生产验证,产品已全量上线**。剩余项集中在 §7「当前待办」——大半在等用户侧三样东西(Docker/Webshare/GROQ key)。
...
>
> **2026-08-26 慢车道生产端到端验证**:消费者管线(C1–C4/C7)完成,用本机跑 `consumer/dist/local.mjs` 当临时消费者——Karpathy 1hr YouTube 视频经「提交 → SQS → 字幕(zh 429 后 en 成功)→ DeepSeek → 锚定 → 回程」产出 12 段地图覆盖全片、锚定 6/6、174 段转写入缓存;失败语义(无字幕+无 whisper key → 显式报错)同验。**当前运行形态**:占位 Lambda 事件源已禁用,慢车道无常驻消费者——需要时本机跑 local.mjs(README 有环境变量清单),否则任务 10 分钟显式超时;真容器上线后恢复事件源。`CONSUMER_TOKEN` 存 `.dev.vars`(gitignored)与 CF secret 同值。
...
| M3 = D 线 + W9–W11 + C1–C4 + F6 | `cdk deploy` 成功;线上投 1 条 B站长视频 + 1 条播客端到端;人为掐断 complete,验证 10 分钟超时显式失败、重投 2 次后落死信队列;缓存命中秒出 | ◐ 主链全验(YouTube 1hr 端到端、超时显式失败、DLQ 37min 落信);**B站 CC / 播客 enclosure 样本待测**(需消费者在跑时投样本) |
| M4 = C5–C7 | 线上 Lambda 实抓 3 条 YouTube 长视频字幕全通;拔掉代理验证兜底路由真的接手 | ⏸ 等 Webshare 注册(C5/C6 是仅剩的未动工项) |
| M5 = P 线 | 纯净 clone 单文件夹 `npm i && npm run dev` mock 跑通(发布门槛);双语 catalog 行 + 主站文案上线 | ✅ 全过 |
| M6 = I 线(v1.5) | 从线上 001 简报任一条一键进 002 并直接提交;有 token 时地图出现追踪器高亮;删掉 token 自动降级、主流程无感 | ⏸ 未动工(刻意排在部署验收后,现可开工) |
...
- [x] **C4 · whisper 兜底**(T5):yt-dlp 下音频 → ffmpeg 32kbps 单声道 → Groq `verbose_json` 拿分段时间戳。*代码就位;活测待 GROQ_API_KEY(本地与 kb-agent 仓均无存值)。无 key 时失败语义已生产验证:显式 complete({error:"没有字幕,whisper 未配置"})*
...
## 7. 当前待办(2026-08-26;原「开工第一天建议」已完成使命删除)

**等用户侧输入的(阻塞项):**

| 需要什么 | 解锁哪些清单项 |
|---|---|
| Docker Desktop 装好/启动 | C1 镜像构建 → D4 换真容器(CDK 改 `DockerImageFunction` + **Architecture X86_64** + env 注入 + 恢复事件源)→ 慢车道免人肉常驻 |
| Webshare Residential 轮换包注册(约 $2–7/月) | C5 代理接入、C6 失败路由 → M4;YouTube 字幕 429 根治 |
| GROQ_API_KEY(console.groq.com 免费档) | C4 whisper 兜底活测;无字幕的视频/播客从「报错」变「能处理」 |
| 告警邮箱拍板 + 部署后点 SNS 确认信 | D6 两条告警(调用量 + DLQ 深度) |
| slug/中文名拍板(现「观影路由 / watch-router」) | 02 风险 7 收口;已上线,再拖改名成本持续上涨 |
| 是否 push(两仓 commits 纯 ahead 无分叉) | 代码落库 |

**不等任何人的(我这边随时可做):**

1. **I 线交互包**(I1–I3,约 2h):001 深读链接 + interop 端点 + 002 用户级高亮——M6 的全部内容,现在可开工
2. **B站 CC / 播客 enclosure 样本测试**(M3 收尾):本机跑起 local.mjs 后投两条样本即可
3. 单测基建(vitest)+ W6/W8 的边界样例(两处清单备注挂着的欠账)
