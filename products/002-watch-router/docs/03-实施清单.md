...
- [x] **N4 · 想法导出**(2026-09-01 站长需求:想法要能拿去喂给别的模型写小红书文案):`buildNotesExport`/`exportFileName` 落在 `src/shared/export.ts`。**导出是纯前端一件事,零新端点**——结果页那条走内存里现成的 `loaded`+`note`;记录页每行的 `.md ↓` 走既有的 `GET /api/result/:key`(它本来就同时回结果、想法和版本戳),拼完 Blob 下载。服务端只加了一处:`/api/history` 每条带回想法条数(`listNotes` 一次 Query 拿全 `NOTE#` 前缀,与 `listReadRecords` 并发),**没记过想法的行不出现导出按钮**,免得给一个按下去是空文件的按钮。

  三条取舍写在 export.ts 顶注里:**只带精简 context**(判决/导读 + 每条想法锚定的那一条要点或那一段,不带全篇详细笔记——AI 产出一多就把亲笔的东西淹了,下游模型会写成复述而不是「我的观点」);**版本对不上的定点想法绝不挂 context**(序号指向的已是另一条要点,只留「记于上一版结果」标注,和结果页 orphanNotes 同一条纪律);**缓存过期照样导**(想法是长期资产,context 是易耗品)。时区由浏览器决定,Worker 里只有 UTC。*20 条单测 + 本地真链路冒烟:提交→记 4 条(含一条故意错版本戳)→导出,错版本戳那条只留标注不带原文*

  **一度做过、当天拆掉的**:记录页顶上的「导出全部想法」(带 `GET /api/export` 与 25 条内容的 context 预算)。站长要的是「对单条记录导出」,一份把所有内容摊平的大文件既不是他要的东西,也让下游模型无从聚焦。拆掉后 `listNotes` 留下来给 `/api/history` 数想法用。
...
## 6.6 事故复盘 · 2026-09-04:一条 YouTube 任务 30 秒猝死,而用户只看到一张白页

**现象**:站长提交一条 YouTube 视频,页面上什么都没有发生,额度却扣了一次。

**查下来是两件独立的事,叠在一起才成了「无声失败」:**

| # | 问题 | 根因 | 修法 |
|---|---|---|---|
| 1 | 任务本身失败 | 容器里的 yt-dlp 钉在 `2026.06.09`,YouTube 把那一版用的 player client 关了。症状是字幕 429、音频 403 | `consumer/Dockerfile` 升到 `2026.08.19`,重建镜像 |
| 2 | 失败没人告诉用户 | `/api/inflight` 对 `status === "failed"` 和 `done` 一视同仁,清掉指针后返回 `{active:false}`;前端 `resumeInflight` 又根本不读 `failed`/`error` 两个字段 | Worker 在清指针前把 `failed`/`error`/`url` 带回去;前端读它,渲染错误并回填原链接 |

**排错时走的弯路,记下来给下一次省时间**:429/403 看着像被限流或封 IP,第一反应是代理有问题。查到 `PROXY_URL` 用的是 Webshare 的**轮换**端点(用户名末段 `-rotate`,每请求换出口 IP),而 YouTube 把媒体直链签给取直链的那个 IP——推理很顺,于是做了「每任务一个粘性会话」的改动。**做完发现没用**:签出来的直链里 `ip=` 就是本次会话的出口,依然 403。真正定死方向的是一个更笨的对照——**本机住宅 IP、完全不走代理,同样 403**;换最新版 yt-dlp,直连和走代理都一次成功。粘性会话那段代码已整个删掉(它没修好任何东西,而且对主路径可能更糟:粘住一个被限流的 IP 会让整单全废,轮换至少每个请求换一次)。

**教训**:平台层的 4xx 别顺着「像什么」查,先做一个能证伪的对照。这条已经写进 `consumer/Dockerfile` 版本那一行的注释里,下次照着做。

**验证**:新镜像走生产代理抓那条视频一次通过(`OK title=… duration=1032`);失败播报用 mock 造了一个 failed 任务实测——`/api/inflight` 回 `{active:false, failed:true, error:"…403 Forbidden", url:"…"}`,再取一次回 `{active:false}`(只播报一次)。

**未决**:失败占用的那次额度目前不退。要不要退是产品决定,没动。
