...
- **Your notes come back out**: thoughts you pin to the verdict, a key point or a chapter export as one Markdown file per item — each note together with the context it was pinned to. From the result page, or straight from a row in the record list. A note written against an older version of a result is exported with a marker and no context rather than silently re-pinned to whatever now sits at that number.
