...
describe("complete() via gateway", () => {
	...
	it("does not retry interactive calls — 让用户再等一轮比直接说失败更糟", async () => {
		// timeoutMs = 向导/refine 这类交互调用。捞花括号那一级已经吃掉绝大多数抖动,
		// 剩下的快速失败
		calls = 0;
		await assert.rejects(complete(gw(), { prompt: "MODE:garbage", system: "S", json: true, timeoutMs: 5000 }));
		assert.equal(calls, 1, "交互调用不该重来");
	});
});
