...
export const aiGuard = createMiddleware<{ Bindings: AppEnv }>(async (c, next) => {
	...
	const denied = await checkAccessCode(c.env, c.req.header("x-access-code"));
	if (denied) {
...
/**
 * Same access-code check without the AI kill switch — for endpoints that
 * spend no tokens but expose personal content (the brief itself, feedback).
 * A daily brief reveals what its owner is focused on, so hosted instances
 * gate reads too, with the same ACCESS_CODE.
 */
...
/** Returns true when ACCESS_CODE is set and `given` matches none of the codes. */
async function checkAccessCode(env: AppEnv, given: string | undefined): Promise<boolean> {
	const codes = (env.ACCESS_CODE ?? "")
		.split(",")
		.map((s) => s.trim())
		.filter(Boolean);
