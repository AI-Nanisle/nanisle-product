...
| 每日简报 | `USER#<email>` | `BRIEF#<YYYY-MM-DD>` | brief(JSON), generatedAt | 重复生成覆盖 |
| 当日额度 | `USER#<email>` | `USAGE#<YYYY-MM-DD>` | gen, ai, **ttl** | 两个计数器同住一条:一次点查出读数,一次条件写完成「占位 + 判上限」(§8.3);TTL 7 天 |
...
主仓 `backend/README.md` 原计划托管产品不持有任何 AI 凭证、一律走网关(LiteLLM 等做虚拟 key、预算、限速)——那是为**匿名公开 demo** 设计的防刷层。2026-08-16 拍板:内测期用 `DEEPSEEK_API_KEY` 直连(Worker 与 Lambda 各配一份 secret)。成立的前提是白名单把匿名流量整个挡在登录外,威胁模型从「任何人刷 token」降到「朋友把额度用超」,后者用每用户每日限额 + `AI_DISABLED` 总闸 + DeepSeek 控制台的用量告警覆盖。**开放注册是回到网关的触发点**,届时产品侧把 provider 换回 `gateway` 即可——但别忘了 Lambda 那一半:CDK 目前只往 Lambda 注 `DEEPSEEK_API_KEY`,没有 `AI_GATEWAY_URL/KEY` 的通道(`shared/ai.ts` 的 gateway 分支两边共用,缺的只是环境变量)。只改 Worker 会变成「服务层走网关、生成侧仍在直连裸 key」,而生成侧才是花钱的大头。
...
内测期用 `DEEPSEEK_API_KEY` 直连,威胁模型只有一个:**已授权的人(或他的脚本、他页面上的重试循环)把额度用超**。匿名流量整个被白名单挡在门外——注册开放,但准入不开放,注册完拿到的只是 403。所以这里的每一道闸都是对着熟人和 bug 设的,不是对着灰产;真正的防刷层是开放注册那天切回网关(§6.2 末)。

**执行原则:先占位,后干活。** 顺序反了这道闸就是假的:旧版立即生成是「读 `genCount` → 转调 Lambda 等 30–60 秒 → Lambda 结束时才自增」,那 30–60 秒里并发打进来的请求全部读到同一个旧计数、全部放行——10 次/日的上限在一次并发爆发面前形同虚设,而这恰恰是全站最贵的端点。现在自增与判上限压在同一个 DynamoDB 条件写里([store-dynamo.ts](../src/shared/store-dynamo.ts) `reserveQuota`),条件不成立时那次自增根本没发生,20 个并发请求里恰好只有前 10 个能过。

| 项 | 值 | 执行点 |
|---|---|---|
| 立即生成 | 10 次 / 人 / 日 | `USAGE#<date>` 的 `gen`:`ADD gen :one` + `ConditionExpression: gen < :limit`,一次原子写同时完成占位与判上限。超了返回 429 与「明早定时生成照常」的文案(不是「失败」——这是预期内的刹车) |
| 编辑侧 LLM 调用 | 60 次 / 人 / 日 | `USAGE#<date>` 的 `ai`,同一把锁。向导三步 + 「对编辑说一句」共用,按请求粗粒度计(`wizard/sources` 偶尔重试的第二次调用不另计):这道闸防的是失控脚本,不是正常人的手速 |
...
**退还只发生在确定没花钱的那条路上。** 转调 Lambda 时 `fetch` 本身抛错 = 请求根本没送到 = 一个 token 都没花,退还刚占的那次;Lambda 返回 502 则不退——它跑过了,钱可能已经花在半截的管线上,退还等于给「失败就疯狂重试」这条真实的烧钱路径放行。

**额度对用户可见。** `GET /api/usage` 回当日两个计数器与上限,页眉常驻一行读数(简报页显示生成额度,配置页两个都显示),用完时生成按钮自己停掉并说明原因。理由很简单:额度是产品的一部分,读者该在用完之前就看见还剩多少,而不是点下去才被 429 拦住。上限值由服务端下发,前端不硬编码——改额度只该动 `shared/store.ts` 的 `QUOTA_LIMITS` 一处。

**代码之外的三道止损。** 上面全部是应用逻辑,成立的前提是这段代码没写错;下面三条不依赖任何应用逻辑:

| 手段 | 位置 | 挡住什么 |
|---|---|---|
| DeepSeek 账户只充一个月的量 + 控制台用量告警 | 运营动作,不在代码里 | key 上有多少钱,最大损失就是多少。唯一一条「限额逻辑写错了也兜得住」的防线 |
| Lambda `reservedConcurrentExecutions: 3` | 主仓 `infra/lib/daily-brief-stack.ts` | 并发硬闸。即使 Worker 侧的额度被绕过,同时也只有 3 条管线在跑,多出来的调用被 Lambda 直接拒掉。日常用量远在其下:定时刊是单次调用内部串行跑完,立即生成是人手速 |
| 调用量告警(>60 次/小时 → SNS 邮件) | 同上,`ALERT_EMAIL` 环境变量启用 | 「所有账号同时异常」「定时刊自己疯狂重试」这类整体失控,只有总量看得见,而看得见的前提是有人被通知到 |
...
**成本**:一期简报是**两次**大输入调用,别只算前一次。①选材:输入大头是候选 excerpt(16 源 × ≤10 条 × 800 字符 ≈ 3–6 万 token);②成稿(enrich):送 ≤10 篇**正文全文**,输入量与选材同一量级甚至更大。按 DeepSeek 的价位两次合计单期几分到一毛人民币;20 人每天一次 + 立即生成余量,月成本百元人民币以内。这个估算仍是纸面数字,以上线首周的真实账单为准——账单和这里对不上时,先怀疑成稿段的正文长度。AWS 侧 $0(§3.2)。
