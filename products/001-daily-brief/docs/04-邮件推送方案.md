...
| SES 沙箱解除申请 | ✅ 已提交(`put-account-details`),等 AWS 批复(约 24h) |
| 收件邮箱验证(沙箱期) | ⏳ 验证邮件已发到 panghd0811@gmail.com,**等用户点确认链接** |
| Cloudflare DKIM 3 条 CNAME | ⏳ **等用户手动添加**(值见 `cdk deploy` 输出 DkimCname1–3;DNS only,别开代理) |
| 产品 Worker 部署(退订端点 + 开关上线) | ⏳ 未部署——工作区还有并行开发中的成稿(enrich)改动,不能夹带半成品上线;随下一次 `npm run deploy` 一起出去 |

DKIM 生效 + 收件邮箱确认之前,Lambda 发信会失败并记入 CloudWatch 日志(简报本身不受影响)——这是预期行为,不是故障。
