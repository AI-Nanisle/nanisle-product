...
> 状态:已拍板(2026-08-18),2026-08-19 代码与 infra 已实施。剩余人工步骤见文末「上线状态」。
...
| 候选 | 结论 | 具体理由 |
|---|---|---|
| **AWS SES** | ✅ 选它 | 生成 Lambda 已在 <AWS 账号 ID>/us-east-1,SES 就在同账号:权限走 Lambda 执行角色加一条 `ses:SendEmail`,不需要新的 API key、不需要管理第二家服务商的账号。调用可以沿用 store-dynamo 已在用的 aws4fetch 直接签名发 HTTP 请求,**零新增 npm 依赖**。费用 $0.10/千封,内测规模约等于零 |
...
| 事项 | 状态 |
|---|---|
| #1–#4 代码(tldr、email.ts、Lambda 发送、退订端点、配置页开关) | ✅ 已实施,`npm run check` 与 38 个单测全过 |
| #5 CDK(EmailIdentity、ses:SendEmail、环境变量) | ✅ 已 `cdk deploy`(栈 NanisleDailyBrief) |
| Worker secret `EMAIL_UNSUB_SECRET` | ✅ 已 `wrangler secret put`;同值备份在产品仓 `.dev.vars`,**cdk deploy 前必须从那里取值设 `$env:EMAIL_UNSUB_SECRET`** |
| SES 沙箱解除申请 | ⏳ 已提交;AWS 标准追问已回复中(发信实践细节),等批复 |
| 收件邮箱验证(沙箱期) | ✅ panghd0811@gmail.com 已验证 |
| Cloudflare DKIM 3 条 CNAME | ✅ 已添加并验证 SUCCESS(2026-08-19) |
| 端到端实测 | ✅ 手动 `{"mode":"all"}`,`emailed:true`,真实收件箱收到 |
| 产品 Worker 部署(退订端点 + 开关上线) | ⏳ 未部署——工作区还有并行开发中的成稿(enrich)改动,不能夹带半成品上线;随下一次 `npm run deploy` 一起出去,**在那之前邮件里的退订链接是 404** |

踩坑备案:SES 沙箱期给 Lambda 的 `ses:SendEmail` 授权资源必须写 `identity/*`——收件人的已验证邮箱本身也是 identity 资源参与鉴权,只授权发件域名会 403 AccessDenied(症状:管理员凭证能发、执行角色不能)。栈里已按此修正并注释。
