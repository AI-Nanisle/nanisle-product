...
## 3.10 P1 · 首个外部用户反馈批(siyang,2026-08-25)

> 第一位非站长用户 siyang huang 的五条反馈,当天全部落地。02 里没有对应 §,设计要点写在条目里。

- [x] **F8 · 冷启动「帮我想」**(反馈 1「不知道一开始要问什么」):向导提问页加一行——用户用一句话说处境(「在做跨境电商独立站」),`POST /api/wizard/suggest` 让编辑当场起草 4 个候选长期问题,点了填进输入框可改。不 hardcode 例题:凭空印的例题帮不了任何具体的人,把处境翻译成长期问题正是编辑的本行(处境是选材的分岔口,理由同 R7)。**选了候选题(原样)开始时,那句处境直接作为 purpose 写进定义**——用户亲口说的不算猜,第 1 步「你在忙什么」追问随之免掉;自己改写了题就不带,留给追问去问。走 userAiGuard 额度 + 轻任务档;mock 模式空列表 + 明说没接 AI。提示词踩坑记录:带具体好例会被 flash 档整段照抄进输出(实测),只能给抽象约束;「自查后再输出」会诱发思考跑飞超 max_tokens,删
- [x] **V 线 · 理解的版本历史与回滚**(反馈 2):
  - **V1 · `intentVersions` 快照**:每次理解被整体重写前(向导继续对话、答用途、「对编辑说一句」改 intent、回滚本身),旧版 segments 原样存档(新在前,≤10 份,`pushIntentVersion` 纯函数有单测;与最近一份逐字相同不记)。档案 02 节和向导第 1 步共用 `IntentHistory` 组件:可查看、可一键「回到这版」;回滚 = 当前版先入历史再整体替换,所以**回滚永远可再回滚**,不存在丢内容的路径
  - **V2 · `wizardMessages` 对话持久化**:向导对话原文落在草稿上(服务端写,`cleanTrackers` 只在 stage 存续期间保留)。修掉一个实际缺陷:旧实现对话历史只在前端内存里,刷新后「继续对话」编辑只看得到第一句,之前聊过的几轮全部失忆——这正是「刷新后没法回溯」的诱因之一
- [x] **R10 · 标签「再给几个」**(反馈 3,只做向导内):第 ② 步加「+ 让编辑再给几个」,`wizard/tags` 端点加 `more:true` 模式——已有标签整体进提示词声明不许重复,返回值**合并**而非覆盖(`mergeTagLists`,超上限时新标签让位:老的都是用户看过留下的)
- [x] **N1(加源侧)· 向导手动加源 + 源需求实时上报**(反馈 4):
  - 向导第 ③ 步补手动输入行(URL + 类目),走既有 `/api/sources/add`(feed 发现 + 真实试抓),名称由 feed 自报标题兜底
  - 手动添加(`origin:"manual"`,向导与来源库两处)的**抓取类失败**即视为一条源需求:Worker 经 SES 实时发邮件给 `DEV_NOTIFY_EMAIL`(谁 / 什么 URL / 为什么失败 / 所属追踪定义),小红书、公众号这类无 feed 平台的需求天然从这条路进来。重复/上限/字段错不发(用户侧误操作不值得一封邮件);通知失败绝不连累加源本身(`waitUntil` + catch);发成功时响应里 `noted:true`,前端如实告诉用户「需求已记给开发者」
  - 部署前提:主仓 `infra` 的 workerPolicy 已加 `ses:SendEmail`(identity/*,授权范围的坑同 E1),**需要 `cdk deploy` 生效**;Worker vars 加 `EMAIL_FROM` / `DEV_NOTIFY_EMAIL`
- [x] **F9 · 「第一期」引导**(反馈 5「不知道试生成是什么意思」):向导完成落回档案页时,若一期都还没有(`dates` 为空且没在生成),档案页顶部出显眼横幅——「定义已生效,下一步:生成你的第一期」,按钮直接点火,并交代「约四五分钟 / 跑完自动跳简报页 / 以后每天自动出刊」。有第一期后永久消失,老用户不受打扰

> 未做,留档备查:理解之外字段(标签/信源)的版本化;源需求的站内汇总视图(邮件够用到需求多起来那天)。
