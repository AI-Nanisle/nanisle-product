...
// 两件事:
//
// 1) **跨仓编码契约**。同一套 token 原语有四份拷贝——主站(nanisle 仓
//    web/lib/sso.ts)签手递票,001 / 002 / 003 各签发并校验自己的会话。四份都
//    是手抄的,而 verifyToken 的设计是「任何一步不对都返回 null,不区分原因」:
//    某天有人改了 payload 的字段或顺序,线上表现是全站静默登录失败,日志里
//    一个字都没有。所以四边各放一份**同向量**的测试把编码钉死。
//
...
//    就是这两个洞的回归。
//
// 对应文件(VECTOR_* 四份必须逐字节相同):
//   nanisle 仓 web/lib/sso.test.ts
//   products/001-daily-brief/src/worker/sso.test.ts
...
const OTHER_AUDIENCE = "daily-brief";
...
describe("sso 契约", () => {
	...
	it("本产品的 AUDIENCE 就是它的挂载 slug", () => {
		assert.equal(AUDIENCE, "weekly-teardown");
	});
});
