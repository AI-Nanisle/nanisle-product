...
describe("complete() via gateway", () => {
	...
	// ---- 补记账的两个回调(2026-09-01 阶段 7 评审建议修 4 / 7)----
	//
	// 这两条钉的是**时机**:回调必须发生在那一发发出去之前。等成功再记账等于
	// 给「失败的重试」开一条免费通道,而那一发的 token 一样烧掉了。所以两条
	// 用例都让**重试本身也失败**——回调仍然必须被调过一次。

	it("onFallback fires before the fallback call, even when that call also fails", async () => {
		const seen: { reason: string; from: string; to: string }[] = [];
		const routed: AiConfig = { ...gw(), fallback: gw() }; // 退路也是同一台,同样 500
		await assert.rejects(complete(routed, { prompt: "MODE:500", onFallback: (i) => seen.push(i) }));
		assert.equal(seen.length, 1, "回落那一发失败了,账照样得记 —— 钱已经花了");
		assert.equal(seen[0]?.from, "gateway");
		assert.equal(seen[0]?.to, "gateway");
		assert.match(seen[0]?.reason ?? "", /./);
	});

	it("onRetry fires before the json retry, even when the retry also fails", async () => {
		calls = 0;
		let firedAt = -1;
		await assert.rejects(
			complete(gw(), { prompt: "MODE:garbage", system: "S", json: true, onRetry: () => (firedAt = calls) }),
		);
		assert.equal(calls, 2, "应当原地重来一次再放弃");
		assert.equal(firedAt, 1, "回调必须在第二发**发出去之前**,不是等它成功之后");
	});

	it("a callback that throws does not take the generation down with it", async () => {
		// 记账挂了是记账的事,不该让这一次生成一起失败
		const routed: AiConfig = { ...gw(), fallback: { provider: "mock" } };
		const r = await complete(routed, {
			prompt: "MODE:500",
			onFallback: () => {
				throw new Error("D1 这会儿写不进去");
			},
		});
		assert.equal(r.provider, "mock");
	...
	it("no retry / no fallback → neither callback fires", async () => {
		const seen: string[] = [];
		await complete(gw(), { prompt: "hello", onFallback: () => seen.push("fb"), onRetry: () => seen.push("rt") });
